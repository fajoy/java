	...
	private ReadLineHandler<StreamHandler> entReqUserName = new ReadLineHandler<StreamHandler>() {
		...
		public boolean action(StreamHandler sender, String line) {
			...
			if (room.getUser(line) != null) {
				...
				room.users.put(user.userName, user);
				user.writeLine("/msg *******************************************");
		...
		private ReadLineHandler<StreamHandler> entReqShowTask = new ReadLineHandler<StreamHandler>() {
			...
			public boolean action(StreamHandler sender, String line) {
				...
				for ( Entry<String, Task> taskItem : TaskStore.tasks.entrySet()) {
					user.writeLine(String.format("/msg Task ID: %s,Task Type: %s",taskItem.getKey(),taskItem.getValue().getClass().getSimpleName()));
				}
				return true;
			}
		};
		*/
		...
		private ReadLineHandler<StreamHandler> entReqTask = new ReadLineHandler<StreamHandler>() {
			...
			public boolean action(StreamHandler sender, String line) {
				...
				GetTask getTask=new GetTask(type,init_str);
				Task task =null;
				try {
				Compute skeleton1 = RMIHelper.getSkeleton(user.userName);
				task = (Task)skeleton1.executeTask(getTask,"");
				}catch (Exception e) {

				}
				if(task==null)
					user.writeLine("/msg create task error.\n");
				else{
					TaskStore.saveTask(id, task);
					user.writeLine(String.format("/msg Task ID: %s,Task Type: %s",id,task.getClass().getSimpleName()));
				}
				return true;
			}
		};*/
		...
		private ReadLineHandler<StreamHandler> entReqRexe = new ReadLineHandler<StreamHandler>() {
			@Override
			public boolean action(StreamHandler sender, String line) {
				final ChatUser user = (ChatUser) sender;
				...
				if(task==null){
					user.writeLine("/msg no such task.\n");
					return true;
				}
				if(!ChatUser.this.room.users.containsKey(target)&&target!="@SERVER"){
					user.writeLine("/msg no such user.\n");
					return true;
				}
				TaskCallBack.execute(task, target, new Callback() {
					@Override
					public void callback(TaskCallBack sender, Object returnValue) {
						user.writeLine(String.format("/msg task %s(%s) execute return:%s",task.getClass().getSimpleName(),taskId,returnValue ));
						user.flush();
					}
				});
				return true;
